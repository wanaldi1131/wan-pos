...
export default function TabPriceLists({ user }: { user: User }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data: ls } = await sb.from('price_lists').select('id, name').order('name')
  ...
  function closeEdit() { setEditList(null); setMsg(null) }
  ...
  function onAddSearch(value: string) {
    setAddSearch(value); setAddUnitId(null); setAddUnits([]); setAddDropOpen(true)
    ...
    debounceRef.current = setTimeout(async () => {
      const { data } = await sb.from('products')
        ...
        .eq('active', true).order('name').limit(8)
      setAddHits((data ?? []) as unknown as ProductHit[])
    }, 300)
  }

  function selectAddProduct(hit: ProductHit) {
    setAddSearch(hit.name); setAddHits([]); setAddDropOpen(false)
    const units = hit.product_units ?? []
    setAddUnits(units); setAddUnitId(units.length === 1 ? units[0].id : null)
  }
  ...
  async function addItem() {
    ...
    const { error } = await sb.from('price_list_items').upsert({
      ...
      price_retail:    parseFloat(addRetail) || null,
      price_toko:      parseFloat(addToko)   || null,
    ...
    loadItems(editList.id); setAdding(false)
  }
  ...
  async function saveItemPrice(unitId: number) {
    ...
    setSavingItem(null); loadItems(editList.id)
  }

  async function removeItem(unitId: number) {
    if (!editList) return
    setRemovingItem(unitId)
    await sb.from('price_list_items').delete()
      .eq('price_list_id', editList.id).eq('product_unit_id', unitId)
    setRemovingItem(null); loadItems(editList.id)
  }
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div>
                      ...
                      <input type="number" inputMode="decimal" value={edit.retail}
                        ...
                        onFocus={e => e.target.select()} placeholder="Pakai master"
        ...
        <div className="space-y-3">
          {lists.map(list => (
            <div key={list.id} className="bg-white border border-gray-200 rounded-2xl px-4 py-3 flex items-center gap-3">
              <p className="flex-1 text-gray-900 font-semibold text-base">{list.name}</p>
